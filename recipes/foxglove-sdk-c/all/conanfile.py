...
class FoxgloveSdkCConan(ConanFile):
    name = "foxglove-sdk-c"
    ...
    def build(self):
        self.run(f"cargo rustc -p foxglove_c {self._build_type_flag} {self._shared_flag} --target-dir {self.build_folder}",
    ...
    def package(self):
        ...
        for path in self._dist_dir.glob("*foxglove.*"):
            if path.suffix == ".d":
                continue
    ...
    def package_info(self):
        ...
        self.cpp_info.libs = ["foxglove"]
        self.cpp_info.includedirs.append("include/foxglove-c")
        if self.settings.os in ["Linux", "FreeBSD"]:
            self.cpp_info.system_libs = ["m"]
        elif is_apple_os(self):
            self.cpp_info.frameworks = ["CoreFoundation", "Security"]
        elif self.settings.os == "Windows":
            self.cpp_info.system_libs = ["Bcrypt", "SChannel", "Crypt32", "Ncrypt"]
